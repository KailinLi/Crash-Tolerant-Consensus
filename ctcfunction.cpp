...
//    std::thread processors[MAX_PROCESSOR];
    ...
    for(int cnt = 0; cnt < ProcessorNum; cnt++) {
        if (cnt < ProcessorNum - CrashNum) {
            ...
            normal[normalCnt]->init (cnt, localVal, ProcessorNum, semb, sema, &info, &vround);
            normal[normalCnt++]->moveToThread (&thread[cnt]);
            ...
            connect (&thread[cnt], &QThread::finished, &thread[cnt], &QThread::deleteLater);
            thread[cnt].start ();
//            processors[cnt] = std::thread(&CTCFunction::NormalProcessor, this, cnt);
            ...
            crash[crashCnt]->init (cnt, localVal, ProcessorNum, semb, sema, &info, &vround);
            crash[crashCnt++]->moveToThread (&thread[cnt]);
